...
g = @(t,x) process_noise_scale * eye(nw);
...
%% Plot a xy projection of the trajectory and the covariance ellipses
figure
plot(init_ref.x(1,:), init_ref.x(2,:), 'k-', 'DisplayName', 'Reference Trajectory', 'LineWidth', 1.5);
hold on
for k = 1:N+1
    plot3sigmaEllipse(init_ref.x(1:2,k), solution_unscaled.P(1:2,1:2,k), 'Color', [.7, .7, .7], 'HandleVisibility', 'off');
end
hold off
grid on
axis equal
xlabel('$x$')
ylabel('$y$')
% title('Lorenz System Trajectory and Covariance Ellipses')
legend('Location', 'best')

exportgraphics(gcf, 'figures/lorenz_transfer_trajectory_and_covariance_ellipses_xy.png') 
