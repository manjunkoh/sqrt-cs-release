...
In order to take advantage of the convex representation of the objective and constraints, we use the SCP algorithm \texttt{SCvx*} \cite{oguriSuccessiveConvexificationFeasibility2023a} to solve \cref{eq:nonconvex_problem_statement}; this ensures that the nonlinear dynamics are obeyed at convergence, and the converged solution is locally optimal. In general, any SCP algorithm can be used to ensure the satisfaction of nonlinear dynamics, but local optimality must be met in order to apply the results of \cref{th:global_optimality,th:local_optimality}. At each iteration, the linearized dynamics \cref{eq:square_root_covariance_propagation_linearized} are imposed with penalization of violations:
...
\end{split}
\end{equation}
where \( \xi_k \in \mathbb{R}^{n(n+1)/2}\) is a slack variable, penalized in the objective function to promote constraint satisfaction. 
...
\begin{align} \label{eq:trust_region_constraint}
	\|\mathrm{vec}(D_X(X_k - \bar{X}_k))\|_\infty &\leq r, \quad k \in [N{-}1]
\end{align}
...
\end{equation}
where \( \lambda_k \in \mathbb{R}^{n(n+1)/2} \) are Lagrange multipliers, \( w > 0\) is the penalty weight, and \(\bm{\xi} := \{\xi_k\}_{k=0}^{N-1}\) are the slack variables from \cref{eq:linearized_dynamics}. For convenience, define \(\bm{z} := \{\bm{v}, \bm{L}, \bm{\mu}, \bm{S} \} \). The augmented objective function is then:
\begin{equation} \label{eq:objective_augmented}
	\mathcal{L}(\bm{z}, \bm{\xi}; w, \bm{\lambda}) := 
	J(\bm{z}) + P(\bm{\xi}; w, \bm{\lambda})
\end{equation}
where \( \bm{\lambda} := \{\lambda_k\}_{k=0}^{N-1} \). In the \texttt{SCvx*} algorithm, $w$ and $\bm{\lambda}$ are updated throughout the iterations to ensure convergence to a feasible and locally optimal solution.
