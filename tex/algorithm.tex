...
The solution process is summarized in Algorithm \ref{alg:sqrt_qr_covariance_steering}. At each iteration, the algorithm solves the convex subproblem \cref{eq:subproblem} to obtain a candidate solution. 
...
When the step is accepted and $|\Delta J^{(i)}| < \delta^{(i)}$ for a stationarity tolerance $\delta^{(i)} > 0$, $\bm{\lambda}$ and $\delta$ are updated. The trust region radius and penalty weight are updated based on the step acceptance ratio, where $\alpha_1 > 1$, $\alpha_2 > 1$, $\beta > 1$, and $0 < \rho_0 < \rho_1 < \rho_2 < 1$ are algorithm parameters.
...
Readers are referred to \cite{oguriSuccessiveConvexificationFeasibility2023a} for more details of the $\texttt{SCvx*}$ algorithm, including its proof of convergence to a local minimum.
