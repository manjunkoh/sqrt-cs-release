...
\begin{figure}[t]
	...
	\end{subfigure}%
	\caption{Comparisons for the obstacle-avoidance case. (\textbf{Left}) Average (in solid lines) and the min-max range (in shaded regions) of the solution times (\textbf{Right}) Objective function divided by the number of nodes $N$}
...
Here, we demonstrate the numerical reliability of the proposed method when the variance of some state components are small. We consider the problem from \cite{oguriChanceConstrainedControlSafe2024a} which uses CS for a spacecraft rendezvous setting. We greatly simplify the problem, using the same system dynamics and boundary condition; we also consider continuous control instead of impulsive control. The problem considers a QoN type objective function with $W_k^x = 0$ and $W_k^u = I$ for all $k$ \cref{eq:objective_function} which is common for optimizing the quantile of the spacecraft's necessary fuel. We emphasize that the parameters of uncertainty are rather realistic for a spacecraft rendezvous setting, and not artificial. The problem parameters are as follows: initial mean position...


The problem parameters are summarized in \cref{tab:cwh_parameters}.
