...
\end{equation}
Recalling that $S_k S_k^\top = P_k$, define the following matrix:
...
\begin{lemma} \label{lemma:rank_X_n_iff_P_succ_0}
	Let $P_{k+1}$, $X_{k+1}$ be defined as in \cref{eq:full_covariance_propagation} and \cref{eq:X_definition}, respectively. Then,
	\begin{equation}
...
Observing \cref{eq:X_definition,eq:square_root_covariance_propagation_nonlinear_feedback_control}, since both $S_k$ and $K_k$ are unknowns, there is a bilinear term \( K_k S_k \); for the sake of convexity, we perform a change of variables and define $L_k = K_k S_k \in \mathbb{R}^{m \times n}$. Then, $K_k$ can be uniquely recovered as $K_k = L_k S_k^{-1}$. The inverse exists since $S_k$ is the Cholesky factor of \( P_k \), which is positive definite by \cref{assump:positive_definite_state}. Note that $L_k L_k^\top$ represents the covariance matrix of the input $u_k$. With this chance of variables, we get
...
\begin{equation} \label{eq:square_root_covariance_propagation_linearized}
	\hspace{-1pt}S_{k+1} \approx \mathrm{qr}(\bar{X}_{k+1}^\top)^\top + \dd R(X_{k+1}^\top - \bar{X}_{k+1}^\top, \bar{Q}_{k+1}, \bar{R}_{k+1})^\top
\end{equation}
...
\begin{equation} \label{eq:objective_deterministic}
	...
	\end{cases}
\end{equation}
The QoN cost is a convex upper bound based on the triangle inequality \cite{oguriChanceConstrainedControlSafe2024a}; \( \mathcal{Q}_{\chi^2_d}(\cdot) \) is the inverse of the chi-squared cumulative distribution function with \( d \) degrees of freedom.

\begin{remark} \label{remark:convexifying_term}
	Although an approximation, the QoN cost is easily handled in the proposed method as a convex function of the variables (also by the block matrix methods \cite{ridderhofChanceConstrainedCovarianceControl2020,oguriChanceConstrainedControlSafe2024a}). The seminal work \cite{liuOptimalCovarianceSteering2025} relies on an assumption on the objective function for convexification, and we are not aware of a convex upper bound for this formulation. Depending on the problem, two differerent approaches can be taken.

	In unconstrained scenarios, a reference matrix $Y_k = L_k L_k^\top$ can be provided for linearizing the square root term. In scenarios where an SCP approach is required due to other constraints (e.g. involving CCs or nonlinear dynamics \cite{kumagaiRobustCislunarLowThrust2025}), we can replace the square-root term with an auxiliary variable $t \geq 0$ and ensure that the nonconvex relationship $\lambda_{\max}(W_k^u Y_k (W_k^u)^\top) \leq t^2$ is imposed iteratively via successive linearization. In either case, such objective functions break the proof of lossless convexification in \cite{liuOptimalCovarianceSteering2025}. To retain the convexification results, i.e. to ensure that the partial of the Lagrangian with respect to \(Y_k\) is strictly positive definite, an extra regularization term \(\eta \trace(Y_k)\) with small \( \eta > 0 \), can be added, as we do in \cite{kumagaiRobustCislunarLowThrust2025}.
\end{remark}
