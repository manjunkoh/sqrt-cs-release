...
\begin{remark} \label{remark:convexifying_term}
	\normalfont Although it is an approximation, the QoN cost is easily handled in the proposed method as a convex function of the variables (also by the block matrix methods \cite{ridderhofChanceConstrainedCovarianceControl2020,oguriChanceConstrainedControlSafe2024a}). The seminal work \cite{liuOptimalCovarianceSteering2025}, due to relying on the EoQ cost for convexification, cannot straightforwardly handle other costs. A natural workaround is to 1. linearize the square root that appears when using the full covariance matrix ($Y_k$ in the notation of \cite{liuOptimalCovarianceSteering2025}) and 2. add a ``convexifying term'' \(\eta \trace(Y_k)\) with small \( \eta > 0 \), to ensure the partial of the Lagrangian with respect to \(Y_k\) is strictly positive definite, as we do in \cite{kumagaiRobustCislunarLowThrust2025} ($\eta$ is originally $\epsilon_Y$ in the reference).
\end{remark}
